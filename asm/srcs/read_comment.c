...
/*   Updated: 2020/02/25 15:09:33 by lubrun           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_comment_tag(char *str)
{
	...
	if (!*str || *str != '"')
	{
		return (write_error(NULL,
			ft_strdup("Token expected:.comment \"[comment]\"\n"), 0, 0));
	}
	return (str - tmp);
}
...
static int	check_comment(char *str, char **name)
{
	...
	{
		return (write_error(NULL,
			ft_strdup("Prog_comment too long!\n"), 0, 0));
	}
...
int			get_comment(t_file *file, char **str)
{
	...
	{
		return (write_error(NULL,
			ft_strdup("Duplicated token .comment\n"), 0, 0));
	}
