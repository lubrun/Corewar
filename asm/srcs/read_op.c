...
/*   Updated: 2020/02/29 01:35:26 by lelajour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			is_label(t_file *file, char *str, int *index)
{
	...
	save = *index;
	sub = ((ft_strchr(str, COMMENT_CHAR) ?
	ft_strsub(str, *index, ft_index(str, COMMENT_CHAR) - *index - 1) : str));
	if (!sub)
		return (-1);
...
int			is_op(t_file *file, char *str, int label)
{
	...
	else if (read_param(file, ft_strsplit(((!tmp) ? str :
	tmp) + ft_strlen(op.name), ","), op))
		ret = 1;
	if (tmp)
		ft_strdel(&tmp);
	return (ret);
