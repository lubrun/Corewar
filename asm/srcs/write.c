...
/*   Updated: 2020/01/29 09:54:29 by lubrun      ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void    		write_dir_ind(t_file *file, t_op op, char *param, t_arg_type type)
{
	...
	ft_memcpy(&file->bytes[file->size], &value, (op.label_size == 1 ||
...
int				write_file(t_file *file)
{
	...
	if (!(output_name = get_output_name(file->file_name)))
		return (0);
	if ((fd = open(output_name, O_CREAT | O_RDWR, S_IRWXU | S_IRWXG | S_IRWXO)) < 0)
		return (0);
	prog_size = file->size;
